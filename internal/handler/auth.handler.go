...
func NewAuthHandler(AuthUsecase interfaces.AuthUsecase, env *env.Env) *AuthHandler {
	return &AuthHandler{
...
func (a *AuthHandler) GoogleCallback(ctx *gin.Context) {
	...
	if stateGoogle == "" {
		ctx.Redirect(http.StatusFound, a.env.DomainFE+"/login?error="+"state Google empty")
		return
	}
	...
	code := ctx.Query("code")
	if code == "" {
		ctx.Redirect(http.StatusFound, a.env.DomainFE+"/login?error="+"code Google empty")
		return
	}
	...
	redirectCookie, _ := ctx.Cookie(constant.GOOGLE_OAUTH_REDIRECT)

	if redirectCookie != "" {
		ctx.Redirect(http.StatusFound, redirectCookie)
		return
	}
