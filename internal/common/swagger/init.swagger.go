...
func Start(ginEngine *gin.Engine, env *env.Env) {
	...
	ginEngine.GET("docs", midBasicAuth, gin.HandlerFunc(func(ctx *gin.Context) {
	...
	ginEngine.GET("/docs.json", midBasicAuth, gin.HandlerFunc(func(ctx *gin.Context) {
		...
		modules := []func(reflector *openapi3.Reflector) error{article, auth, user}
		for _, module := range modules {
			err := module(reflector)
			if err != nil {
				ctx.JSON(400, "Loi")
				return
			}
